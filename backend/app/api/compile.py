...
async def background_compile(run_id: str, requirements: str, options, db: AsyncSession):
    # This would normally update DB status as it progresses
    # Simplified here to just run the pipeline and update at the end
    try:
        response = pipeline.compile_sync(requirements, options, run_id=run_id)
...
async def compile_requirements(
    ...
    if sync:
        # Run synchronously
        response = pipeline.compile_sync(request.requirements, request.options, run_id=run_id)
        ...
        # Run in background
        background_tasks.add_task(background_compile, run_id, request.requirements, request.options, db)
