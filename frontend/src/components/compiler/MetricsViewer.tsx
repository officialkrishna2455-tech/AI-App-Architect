    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass p-4 rounded-xl flex flex-col justify-center items-center text-center">
          ...
          <div className="text-2xl font-bold text-white">{formatDuration(metrics.total_latency_ms)}</div>
        </div>
        <div className="glass p-4 rounded-xl flex flex-col justify-center items-center text-center">
          <div className="text-sm text-slate-400 mb-1">Validation Pass Rate</div>
          <div className="text-2xl font-bold text-accent-cyan">{(metrics.validation_pass_rate * 100).toFixed(0)}%</div>
        </div>
        <div className="glass p-4 rounded-xl flex flex-col justify-center items-center text-center">
          <div className="text-sm text-slate-400 mb-1">Auto-Repairs</div>
          <div className="text-2xl font-bold text-warning">{metrics.repair_count}</div>
        </div>
        <div className="glass p-4 rounded-xl flex flex-col justify-center items-center text-center">
          <div className="text-sm text-slate-400 mb-1">Simulation Pass Rate</div>
      ...
      {validation_report?.issues && validation_report.issues.length > 0 && (
        <div className="glass rounded-xl border border-white/10 p-4">
          <h3 className="font-bold text-lg mb-4">Validation Issues & Repairs</h3>
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
            {validation_report.issues.map((issue, idx) => {
              ...
              return (
                <div key={idx} className="bg-navy-900/50 rounded p-3 border border-white/5 flex gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm flex gap-2 items-center">
                      ...
                      <span className="text-slate-300">{issue.layer} layer</span>
                    </div>
                    <div className="text-slate-400 text-sm mt-1">{issue.message}</div>
                    {wasRepaired && (
                      <div className="text-success text-xs mt-2 font-medium bg-success/10 inline-block px-2 py-1 rounded">
